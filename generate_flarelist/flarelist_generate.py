...
def fetch_operational_flare_list(tstart, tend, save_csv=True):
    """
...
def filter_and_associate_files(flare_list, local_files_path, threshold_counts=1000, save_csv=True):
    """
...
def estimate_flare_locations(flare_list_with_files, save_csv=True):
    """
...
def merge_and_process_data(flare_list_with_locations, save_csv=True):
    """
    ...
    # Column renaming for final data
...
def get_flares(tstart, tend, local_files_path):
    ...
    # step 2: filter to counts about 1000 and get list of cpd files associated with each
